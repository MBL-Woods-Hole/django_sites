...
class IlluminaAdaptorRefIts1Temp(models.Model):
    illumina_adaptor = models.CharField(max_length=3)
    illumina_run_key = models.CharField(max_length=5)
    illumina_index = models.CharField(max_length=6)
    dna_region = models.CharField(max_length=32)
    domain = models.CharField(max_length=8, blank=True, null=True)

    class Meta:
        managed = False
        db_table = 'illumina_adaptor_ref_its1_temp'
        unique_together = (('illumina_adaptor', 'dna_region', 'domain'),)
...
class RunPrimer(models.Model):
    run_primer_id = models.SmallIntegerField(primary_key=True)
    run_key = models.ForeignKey(RunKey, models.DO_NOTHING, blank=True, null=True)
    run = models.ForeignKey(Run, models.DO_NOTHING, blank=True, null=True)
    lane = models.IntegerField()
    primer = models.ForeignKey(Primer, models.DO_NOTHING, blank=True, null=True)
...
class SequenceIll(models.Model):
    sequence_ill_id = models.AutoField(primary_key=True)
    sequence_comp = models.TextField(unique=True)

    class Meta:
        managed = False
        db_table = 'sequence_ill'


class SequencePdrInfoIll(models.Model):
    sequence_pdr_info_ill_id = models.AutoField(primary_key=True)
    run_info_ill = models.ForeignKey(RunInfoIll, models.DO_NOTHING)
    sequence_ill = models.ForeignKey(SequenceIll, models.DO_NOTHING)
    seq_count = models.IntegerField()
    updated = models.DateTimeField()

    class Meta:
        managed = False
        db_table = 'sequence_pdr_info_ill'
        unique_together = (('run_info_ill', 'sequence_ill'),)


class SequenceUniqInfoIll(models.Model):
    sequence_uniq_info_ill_id = models.AutoField(primary_key=True)
    sequence_ill = models.ForeignKey(SequenceIll, models.DO_NOTHING, unique=True)
    taxonomy = models.ForeignKey('Taxonomy', models.DO_NOTHING)
    gast_distance = models.DecimalField(max_digits=7, decimal_places=5)
    refssu_id = models.IntegerField()
    refssu_count = models.IntegerField()
    rank = models.ForeignKey('Rank', models.DO_NOTHING)
    refhvr_ids = models.TextField()
    updated = models.DateTimeField()

    class Meta:
        managed = False
        db_table = 'sequence_uniq_info_ill'


class Taxonomy(models.Model):
    taxonomy_id = models.AutoField(primary_key=True)
    taxonomy = models.CharField(unique=True, max_length=300)
